            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, "blue"):
                        ...
                        # use token-mean entropy for comparability
                    ...
                    if "rollout_log_probs" in batch.batch.keys():
                        with torch.no_grad():
                            # rollout_log_probs is padded with -1, so we need to mask them out
                            # however, log_prob may be -1.0 for valid tokens, use response_mask instead.
                            # log_prob_mask = torch.logical_not(batch.batch["rollout_log_probs"] == -1.0)
                            log_prob_mask = batch.batch["response_mask"].bool()
                            log_prob_diff = (
                                batch.batch["rollout_log_probs"][log_prob_mask].float()
                                - batch.batch["old_log_probs"][log_prob_mask].float()
                            )
                            ...
                            metrics.update(
                                {
                                    "train/vllm_kl": vllm_kl.detach().item(),
