...
@register("roo-lp")
class RooLpRewardManager(AbstractRewardManager):
    """The reward manager."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.roo_lp_cfg = roo_lp_cfg
        assert self.roo_lp_cfg is not None, "roo_lp_cfg must be provided"
        self.roo_lp_alpha = self.roo_lp_cfg.get("alpha", 0.1)
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        scores_list = []
        uid2correct_response_length: dict[str, list[int]] = defaultdict(list)
        ...
        for i in range(len(data)):
            ...
            if score == 1:
                ...
                assert uid is not None, "uid must be provided for correct responses"
                uid2correct_response_length[uid].append(valid_response_length.item())
        ...
        assert scores_set.issubset({0, 1}), f"Roo-LP only supports binary scores, but got {scores_set}"
        for i in range(len(data)):
            score = scores_list[i]
            data_item = data[i]  # DataProtoItem
            uid = data_item.non_tensor_batch.get("uid", None)
            assert uid is not None, "uid must be provided for Roo-LP reward computation"
            if score == 1:
                correct_response_lengths = uid2correct_response_length[uid]
                avg_correct_response_length = np.mean(correct_response_lengths)
                std_correct_response_length = np.std(correct_response_lengths)
                normalized_length_diff = (valid_response_length_list[i] - avg_correct_response_length) / (
                    std_correct_response_length + 1e-8
                )

                def sigmoid(x):
                    return 1 / (1 + np.exp(-x))

                overlong_reward = -self.roo_lp_alpha * sigmoid(normalized_length_diff)
            else:
                overlong_reward = 0.0
