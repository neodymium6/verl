...
def run_test(
    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            signal.alarm(timeout)
            try:
                assert isinstance(method_name, str), "Function name must be a string for call-based testing."
                returns = grade_call_based(
                    code=test,
                ...
                if returns is None:
                    return [-4], {
                        "error_code": -4,
                        "error_message": "Error during testing: Compilation or function retrieval failed.",
                    }
